...
return(
  ...
  <div className="w-11/12 md:w-9/12 p-4 rounded-xl" style={{backgroundImage:"linear-gradient(to top, #a18cd1 0%, #fbc2eb 100%)", boxShadow:"rgba(0, 0, 0, 0.17) 0px -23px 25px 0px inset, rgba(0, 0, 0, 0.15) 0px -36px 30px 0px inset, rgba(0, 0, 0, 0.1) 0px -79px 40px 0px inset, rgba(0, 0, 0, 0.06) 0px 2px 1px, rgba(0, 0, 0, 0.09) 0px 4px 2px, rgba(0, 0, 0, 0.09) 0px 8px 4px, rgba(0, 0, 0, 0.09) 0px 16px 8px, rgba(0, 0, 0, 0.09) 0px 32px 16px"}}>
  <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-5xl text-center font-bold p-3">Password Generator</h1>
    <div className="flex items-center gap-3 justify-center">
    <div style={{backgroundColor:"#A9A9A9"}} className="text-center h-8 md:h-12 flex justify-center items-center w-11/12 mt-3">
    <p className="text-white text-xl md:text-3xl">{password}</p>
    
    </div>
    <div>
    <button onClick={copyToClipboard}><i className="fa-solid fa-copy text-3xl md:text-5xl cursor-pointer"></i></button>
    </div>
    

    </div>
   <div className="text-center p-5">
   <button onClick={generatePassword} className="border p-1 px-5 rounded-md bg-green-500 hover:bg-green-700 text-sm sm:text-lg">Generate Password</button>
   </div>
   ...
   <div className="flex flex-col p-3 md:p-10 text-xs md:text-sm lg:text-lg">
    <label htmlFor="">
      <input type="checkbox" checked={includeUppercase} onChange={(e)=>setIncudeUppercase(e.target.checked)} />
      Include Uppercase
    </label>
   ...
   <input type="number" value={characters} onChange={(e)=>setCharscters(Number(e.target.value))} className="border border-black w-12 text-center rounded-lg"/>
